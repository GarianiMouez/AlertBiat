...
import { quantaraProps } from "../../types/quantaraProps";
...
const url = "http://localhost:3085/quantara/getquantara/";

const AlertLineChart: React.FC<quantaraProps> = (props: quantaraProps) => {
  ...
  useEffect(() => {
    console.log("param from useEffect", props.param);
    setInterval(() => {
  ...
  useEffect(() => {
    if (!listening) {
      const events = new EventSource(url + props?.param);
